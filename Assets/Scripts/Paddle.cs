...
public class Paddle : MonoBehaviour {
	...
	void Update () {
		...
		if (NewGame.onePlayer) {
			ball = GameObject.FindGameObjectWithTag ("ball");
			rbBall = ball.GetComponent<Rigidbody> ();
			playerPos = new Vector2(gameObject.transform.position.x, Mathf.Clamp(gameObject.transform.position.y + Input.GetAxis ("VerticalTwo") * 0.5f, -5.3f, 5.3f));
			gameObject.transform.position = playerPos;
			for (int i = 0; i < Input.touchCount; i++) {
				if(Input.touchCount > i && Input.GetTouch(i).phase == TouchPhase.Moved && Input.GetTouch(i).position.x <= 600){
			...
			gameObject.transform.position = playerPos;
			for (int i = 0; i < Input.touchCount; i++) {
				if(Input.touchCount > i && Input.GetTouch(i).phase == TouchPhase.Moved && Input.GetTouch(i).position.x <= 900){
					Vector2 touchDeltaPosition = Input.GetTouch (i).deltaPosition;
					transform.Translate(0, Mathf.Clamp(touchDeltaPosition.y * 0.057f, -5.3f, 5.3f), 0);
					Vector2 playerPos = new Vector2 (transform.position.x, transform.position.y);
					if (playerPos.y > 5.3f) {
